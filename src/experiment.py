...
def get_file_id(filename: str) -> str:
    # X...._auth.jpg / X...._tamp.jpg -> X....
    return filename.split("_")[0]


def load_roi_from_sroie(project_root: Path, file_id: str) -> Optional[Tuple[int, int, int, int]]:
    """
    Cari box file di train/box lalu test/box.
    ROI diambil dari:
      - method single: rect total_box
      - method pair  : rect value_box
    """
...
def _collect_scores_for_threshold(
    project_root: Path,
    data_dir: Path,
    mode: str,
    baseline_thr: float,
    ...
    """
    Kumpulkan score untuk kelas authentic saja, untuk data-driven thresholding.
    """
    ...
    with labels_path.open("r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if mode == "baseline_global":
                ...
                if roi is None:
                    continue
                # gunakan thr dummy dulu saat koleksi score
                res = morphcc_roi(image, roi, thr=999999.0, **morph_params)
                scores_auth.append(res.score)
            else:
                raise ValueError("Unknown mode for threshold collection")
...
def choose_threshold(scores_auth: List[float], method: str = "mad15") -> float:
    """
    Threshold selection yang lebih sensitif (cocok untuk UAS).
    """
    if not scores_auth:
        return 0.0
...
def run(
    ...
    threshold_tuning:
      - jika True, thr akan dihitung dari score authentic (data-driven)
      - jika False, gunakan baseline_thr atau default thr pada morphcc
    """
    morph_params = morph_params or {"ksize": 3, "min_area": 10}
    ...
    # ----------------------------
    # 1) Determine threshold (optional)
    # ----------------------------
    tuned_thr = None
    if threshold_tuning:
        scores_auth = _collect_scores_for_threshold(
    ...
    # ----------------------------
    # 2) Run inference + save artifacts
    # ----------------------------
    y_true, y_pred = [], []
    ...
    with labels_path.open("r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if mode == "baseline_global":
                ...
                thr = float(tuned_thr) if tuned_thr is not None else 6.0
                res = morphcc_roi(image, roi, thr=thr, **morph_params)

                # evidence outputs
                save_overlay_morphcc(image, roi, res.artifacts["components"], overlays_dir / f"{fn}.png")
                save_mask(res.artifacts["mask"], masks_dir / f"{fn}_mask.png")
    ...
    metrics.update({
        ...
        "threshold_value": float(tuned_thr) if tuned_thr is not None else (baseline_thr if mode == "baseline_global" else 6.0),
...
if __name__ == "__main__":
    ...
    run(
        ...
        run_id="exp1",
        baseline_thr=18.0,
        threshold_tuning=True,       # ON: data-driven threshold
        threshold_method="p95",      # "p95" atau "mad3"
    )
    ...
    run(
        ...
        run_id="exp1",
        threshold_tuning=True,       # ON: data-driven threshold
        threshold_method="mad15",      # "p95" atau "mad3"
        morph_params={"ksize": 3, "min_area": 10},
    )
